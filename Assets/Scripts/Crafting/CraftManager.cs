...
public class CraftManager : MonoBehaviour {
	...
	public void createObject(int productID, int productAmount, int[] componentID, int[] componentAmount){
		...
		for (int k = 0; k < componentID.Length; k++) { // looping on the components
			if (!isCreatable) { // Already found a lack of component
				break;
			}
			bool isCreatableAux = true;
			foreach (Item key in inventory.Keys) // looking for the component in the inventory items List
			{
				if (key.id == componentID [k]) { // Component found
					if (inventory [key] < componentAmount [k]) { // Not enough materials
		...
		if (isCreatable) { // Enough of every components
